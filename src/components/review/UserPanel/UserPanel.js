...
export default function UserPanel({ entriesInfo, actions, currentAction, adjacentEvents, location, params, showAdjacentEvents, saved, eventId, comment }) {
    ...
    return (
        <div className="p-5 border-primary border-2 mx-2 rounded-sm">
            ...
            {actions.length === 0 ? <></> : <ActionContainer actions={actions} currentAction={currentAction?.name} comment={comment}></ActionContainer>}
